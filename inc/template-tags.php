 ...
 * NOVA FUNKCIYA:
 * 
 * ako post e redaktiran
 * pokazva avatar na choveka redaktiral post
 * i label [updated]
 */
function prevodi_posted_on() {
	$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published updated" datetime="%3$s"><strike>%2$s</strike> %4$s</time>';
	}
	...
	$byline = sprintf(
        ...
        $author = get_the_author(); //user koito napisal
        $editor = get_the_modified_author(); //user koito redaktiral
        
        if ($author !== $editor) { //ako dvamata sa razlichni to togava izkarai i dvamata, ako sa ednakvi - samo ediniya
            $editbyline = sprintf(
                    $editor . ' <label class="prevodi-label" id="green-label"> редактор</label>'
                    );
            $byline .= ' '.$editbyline; //dobavya redaktora kym output na avtora}
        } 
